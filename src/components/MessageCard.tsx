...
const MessageCard: React.FC<MessageCardProps> = ({ message, onListened }) => {
  ...
  return (
    <div className={`relative flex flex-col gap-4 rounded-3xl bg-gray-900/40 p-5 border transition-all duration-300 ${
      !hasPlayed ? 'border-purple-500/30' : 'border-gray-800'
    }`}>
      {/* Unread Badge */}
      {!hasPlayed && (
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-gray-600">
            {message.duration_seconds} seconds
          </span>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          {isExpanded && (
            <div className="rounded-xl bg-black/30 p-4 animate-in fade-in slide-in-from-top-2">
              <p className="text-sm text-gray-400 italic leading-relaxed leading-relaxed">
                "{message.transcript}"
              </p>
            </div>
          )}
